...
async function askBackend(value) {
  ...
  try {
    ...
    if (!r.ok) {
      throw new Error(data?.error || `Backend returned HTTP ${r.status}`);
    }
...
function runLocalAction(value) {
  ...
  if (text === 'give me a system briefing') {
    return 'All systems nominal. Core services are online, memory is synchronized, and the automation bridge is standing by.';
  }

  if (text === 'what should i focus on next?') {
    return 'Your highest-leverage next move is to finish the tomorrow briefing. I can structure it around priorities, blockers, and decisions.';
  }

  if (text === 'run a diagnostics check') {
    return 'Diagnostics complete. CPU, memory, network, voice interface, and memory index are operating within normal parameters.';
  }

  if (text === 'play music') {
    return 'Certainly. Tell me an artist, song, playlist, or genre and I can open a music search for it.';
  }
...
async function send(text) {
  ...
  try {
    ...
    const reply = localReply || await askBackend(value);
...
document.querySelectorAll('[data-command]').forEach(b =>
  b.addEventListener('click', () => send(b.dataset.command))
);
...
function startListening() {
  ...
  recognition.onerror = e => {
    ...
    state.textContent = 'Voice input error';
  };
...
setInterval(() => {
  ...
  if (cpu) {
    cpu.textContent = n + '%';
    cpu.nextElementSibling.firstElementChild.style.width = n + '%';
  }
}, 3000);
